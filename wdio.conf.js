...
exports.config = {
    ...
    suites: {
        Login: [
            './test/specs/LoginWithInValidUserID.spec.js',
            './test/specs/LoginWithValidUserID.spec.js'
        ],
        calendar: [
            './test/specs/SearchCalendar.spec.js',
            './test/specs/NewCalendar.spec.js',
            './test/specs/DeleteCalendar.spec.js'
        ]
    },
    // ...
    ...
    services: [
        [
            ...
            outputDir: `./${OUTPUT_FOLDER}/chrome`, // overwrites the config.outputDir
            args: ['--silent']
        }], 
        // [
        //     'geckodriver',
    ...
    reporters: [
        ...
        'spec', // standard wdio reporter
        'json',
        ['timeline', { 
            outputDir: `./${OUTPUT_FOLDER}/timeline`,
            embedImages: false,
            screenshotStrategy: 'before:click'
        }],
        [video, {
            outputDir: `./${OUTPUT_FOLDER}/video`,
     ...
     */
     before: function (capabilities, specs) {
        ...
        browser.addCommand("setValueByKeys", async function (value) {
            await browser.keys('\uE010' + '\uE003'.repeat((await this.getValue()).length)+value) 
        }, true)

        browser.addCommand("updateValueByKeys", async function (newValue) {
            await this.click()
            const oldValue = await this.getValue()
            if (oldValue !== newValue) {
                await browser.keys('\uE010' + '\uE003'.repeat(oldValue.length)+newValue) 
            }
        }, true)
    },
    /**
    ...
    afterTest: function(test, context, { error, result, duration, passed, retries }) {
        // Not needed for timeline, likely deposited by allure
        // if (!passed) {
